...
int main(void)
{
    ...
    long long unsigned beg_over, end_over;
     
    beg_over = milliseconds_now();
    ...
    end_over = milliseconds_now();

    printf("Time %llu ms, Time_over %llu ms, Time_sort %f ms\n",
            end - beg,
            end_over - beg_over,
            ((end - beg) - (end_over - beg_over)) / (double) N_REPS);
