...
double mid_cmp_tree(node_t *root)
{
    size_t elems, cmps;
    elems = count_elems_tree(root);
    cmps = count_comparison_tree(root, 1);

    return cmps / (double) elems;
}

void act_count_elems_tree(node_t *root, void *param)
{
    size_t *count = param;
    if (root != NULL)
        (*count)++;
...
size_t count_elems_tree(node_t *root)
{
    size_t count = 0;
    each_nlr_tree(root, act_count_elems_tree, &count);
    return count;
}

size_t count_comparison_tree(node_t *root, size_t height_now)
{
    if (root == NULL)
        return 0;
    
    size_t lh, rh;
    lh = count_comparison_tree(root->left, height_now + 1);
    rh = count_comparison_tree(root->right, height_now + 1);
...
size_t mem_bytes_tree(node_t *root)
{
    size_t elems = count_elems_tree(root);

    return sizeof(node_t) * elems;
}
