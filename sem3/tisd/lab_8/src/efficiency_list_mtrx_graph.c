...
unsigned long long measure_time_mtrx_graph(void *data_struct)
{
    ...
    for (size_t i = 0; i < COUNT_CALCS; i++)
    {
        find_euler_path_mtrx_graph(graph, &path, &len_path);
        free(path);
    }
    end = ms_now();
...
unsigned long long measure_time_list_graph(void *data_struct)
{
    ...
    for (size_t i = 0; i < COUNT_CALCS; i++)
    {
        find_euler_path_list_graph(graph, &path, &len_path);
        free(path);
    }
    end = ms_now();
...
int make_compare_table_list_mtrx_graph(listg_t *list_graph, mtrxg_t *mtrx_graph)
{
    ...
    int len_t = 10000;
    long long for_time, list_epath_time, mtrx_epath_time;
    ...
    rc = gettime_list_mtrx_graph(measure_time_list_graph, list_graph, t, len_t, &list_epath_time);
    assert(rc == OK); 
    list_epath_time = minus_for_time(list_epath_time, for_time);

    rc = gettime_list_mtrx_graph(measure_time_mtrx_graph, mtrx_graph, t, len_t, &mtrx_epath_time);
    assert(rc == OK); 
    mtrx_epath_time = minus_for_time(mtrx_epath_time, for_time);

    koef_time = list_epath_time / (double) mtrx_epath_time;

    ft_printf_ln(tab, "find epath path (mks)|%lld|%lld|%.2lf", list_epath_time, mtrx_epath_time, koef_time);

    list_bytes = bytes_list_graph(list_graph);
    mtrx_bytes = bytes_mtrx_graph(mtrx_graph);
    koef_bytes = (double) list_bytes / (double) mtrx_bytes;
