...
int input_graphs(listg_t **list_graph_out, mtrxg_t **mtrx_graph_out)
{
    ...
    if (scanf("%d", &edges) != 1 || edges < 1)
        return print_err(ERR_COUNT_EDGE);
...
int act_mtrx_graph(mtrxg_t *mtrx_graph)
{
    ...
    printf("Условие Дирка ");
    if (dirk_condition_mtrx_graph(mtrx_graph))
        printf("выполнено\n");
    else
        printf("не выполнено\n");

    printf("Условие Оре ");
    if (ore_condition_mtrx_graph(mtrx_graph))
        printf("выполнено\n");
    else
        printf("не выполнено\n");

    int *ham_path_mtrx_graph = NULL;
    rc = find_ham_path_mtrx_graph(mtrx_graph, &ham_path_mtrx_graph);
    if (rc != OK && rc != ERR_NO_HAM_PATH)
        goto ham_free;

    if (rc == ERR_NO_HAM_PATH)
    {
        printf("В данном графе нет гамильтонова пути\n");
        rc = OK;
    }
    else if (rc == OK)
        print_ham_path_mtrx_graph(mtrx_graph, ham_path_mtrx_graph);
    
    ham_free:
    free(ham_path_mtrx_graph);
...
int act_list_graph(listg_t *list_graph)
{
    ...
    printf("Условие Дирка ");
    if (dirk_condition_list_graph(list_graph))
        printf("выполнено\n");
    else
        printf("не выполнено\n");

    printf("Условие Оре ");
    if (ore_condition_list_graph(list_graph))
        printf("выполнено\n");
    else
        printf("не выполнено\n");

    int *ham_path_list_graph = NULL;
    rc = find_ham_path_list_graph(list_graph, &ham_path_list_graph);
    if (rc != OK && rc != ERR_NO_HAM_PATH)
        goto ham_free;
    ...
    if (rc == ERR_NO_HAM_PATH)
    {
        ...
        print_ham_path_list_graph(list_graph, ham_path_list_graph);
    
    ham_free:
    free(ham_path_list_graph);
