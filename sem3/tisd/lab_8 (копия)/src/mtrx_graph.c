...
int find_euler_path_mtrx_graph(mtrxg_t *graph, int **epath)
{
    mtrxg_t *copy_graph = copy_mtrx_graph(graph);
    if (copy_graph == NULL)
        return ERR_MEM;
    
    stack_t *path = create_stack(graph->count_v * (graph->count_v - 1) / 2);
    if (path == NULL)
    {
        free(copy_graph);
        return ERR_MEM;
    }
    stack_t *st = create_stack(graph->count_v * (graph->count_v - 1) / 2);
    if (st == NULL)
    {
        free(copy_graph);
        free(path);
    ...
    int start_id = find_less_connected_odd_v(graph);
    if (start_id == -1)
        start_id = 0;
    ...
    while (!is_empty_stack(st))
    {
        x = *st->ps;
        y = -1;
        for (size_t i = 0; i < graph->count_v && y == -1; i++)
            if (graph->arr[x][i] == 1)
                y = i;
        if (y != -1)
        {
