...
# 6*p3*x+(-2*a2-2*a1-2*a0)*p3+2*p2
def second_derivative_newton_n3_fromula(tab_sep_diff, x):
    return (6 * x - 2 * tab_sep_diff[2][0] - 2 * tab_sep_diff[1][0] - 2 * tab_sep_diff[0][0]) * tab_sep_diff[0][4] + \
        2 * tab_sep_diff[0][3]


def second_derivative_newton_n3_from_short_data(data, x, printing=False):
    tab = build_tab_sep_diff_n_newton(data)
    if printing:
        print('Полином Ньютона')
        print_tab_sep_diff_newton(tab)

    sec_der_polynom = second_derivative_newton_n3_fromula(tab, x)
    return sec_der_polynom


def second_derivative_newton_n3(big_data, x, printing=False):
    n = 3
    short_data = get_short_data(big_data, x, n + 1)
    y__ = second_derivative_newton_n3_from_short_data(short_data, x, printing)
    return y__
